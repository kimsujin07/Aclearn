package com.ezen.aclearn.common;
...
public class FileUtils {
	...
	public static Map<String, String> parseFileInfo(MultipartFile file, String attachPath) throws IOException {
		Map<String, String> fileMap = new HashMap<String, String>();
		
		String boardOriginFileNm = file.getOriginalFilename();

		// 같은 파일명을 업로드했을 때 덮어써지지 않게 하기 위한 실제 업로드되는 파일명 설정
		SimpleDateFormat fommater = new SimpleDateFormat("yyyyMMddHHmmss");
		Date nowDate = new Date();
		String nowDateStr = fommater.format(nowDate);
		...
		if (type != null) {
			if (type.startsWith("image")) {
				fileMap.put("boardFileCate", "img");
			} else {
				fileMap.put("boardFileCate", "etc");
			}
		} else {
			fileMap.put("boardFileCate", "etc");
		}

		fileMap.put("boardFileNm", boardFileNm);
		fileMap.put("boardOriginFileNm", boardOriginFileNm);
